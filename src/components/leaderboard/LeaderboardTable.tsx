            ...
            <>
              ...
              {hasPlayed.length > 0 && (playerOfMonth || (diguKing && diguKing.gin_count > 0)) && (
                <div className="podium-section">
                  ...
                  {/* WhatsApp Share Hall of Fame */}
                  {(playerOfMonth || (diguKing && diguKing.gin_count > 0)) && (() => {
                    const period = month || new Date().toLocaleString('en',{month:'long',year:'numeric'});
                    const nick = (p: typeof playerOfMonth) => p?.nickname || p?.name?.split(' ')[0] || '';
                    let txt = `🏆 DIGU LEAGUE — HALL OF FAME\n${period}\n\n`;
                    if (playerOfMonth) txt += `🥇 Champion: ${nick(playerOfMonth)}\n   ${playerOfMonth.league_points} pts · ${playerOfMonth.win_rate}% win rate · ${playerOfMonth.matches_won} wins\n\n`;
                    if (diguKing && diguKing.gin_count > 0) txt += `👑 Digu King: ${nick(diguKing)}\n   ${diguKing.gin_count} digus · ${diguKing.win_rate}% win rate\n\n`;
                    txt += `Play Smart. Win Big. Reign Supreme. ♠♥♣♦`;
                    return (
                      <a href={`https://wa.me/?text=${encodeURIComponent(txt)}`} target="_blank" rel="noopener noreferrer"
                        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', margin: '0.875rem auto 0', padding: '0.625rem 1.25rem', background: 'rgba(37,211,102,0.1)', border: '1px solid rgba(37,211,102,0.3)', borderRadius: 30, color: '#25D366', fontWeight: 700, fontSize: '0.875rem', textDecoration: 'none', width: 'fit-content', transition: 'all .2s' }}>
