      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {players.map(p => (
          <div key={p.id} className="card pl-card" style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', padding: '0.5rem 0.7rem', border: '1.5px solid rgba(212,175,55,0.45)', boxShadow: '0 0 14px rgba(212,175,55,0.1), inset 0 1px 0 rgba(212,175,55,0.08)', position: 'relative', overflow: 'hidden', flexWrap: 'wrap' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', marginTop: '0.15rem', flexWrap: 'wrap' }}>
                    {nickEditId === p.id ? (
                      ...
                      <button
                        ...
                        style={{ background: 'none', border: p.nickname ? '1px dashed rgba(212,175,55,0.45)' : '1px dashed var(--border)', borderRadius: 4, padding: '0.1rem 0.4rem', cursor: 'pointer', fontSize: '0.6rem', fontFamily: "'Montserrat', sans-serif", color: p.nickname ? 'var(--gold, #FFD700)' : 'var(--text-muted)', fontWeight: p.nickname ? 700 : 400, textTransform: p.nickname ? 'uppercase' : 'none', letterSpacing: p.nickname ? '0.03em' : 0 }}
                      >
            ...
            {editingId !== p.id && (
              <div className="pl-actions" style={{ display: 'flex', gap: '0.375rem', flexShrink: 0, alignItems: 'center' }}>
