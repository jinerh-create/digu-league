            ...
            <tbody>
              {rows.map(r => (
                <tr key={r.id} className={r.winnerId === p1Id ? 'row-win-a' : 'row-win-b'}>
                  ...
                  <td className="ss-score-cell" style={{ color: 'var(--team-a)', cursor: (!isAuthed) ? 'default' : 'pointer' }}
                    onClick={() => { if (!isAuthed) return; setEditingScore({ gameId: r.id, side: 'p1' }); setScoreDraft(String(isTeam ? r.t1Score : (r.p1Pts ?? 0))); }}>
                  ...
                  <td className="ss-score-cell" style={{ color: 'var(--team-b)', cursor: (!isAuthed) ? 'default' : 'pointer' }}
                    onClick={() => { if (!isAuthed) return; setEditingScore({ gameId: r.id, side: 'p2' }); setScoreDraft(String(isTeam ? r.t2Score : (r.p2Pts ?? 0))); }}>
